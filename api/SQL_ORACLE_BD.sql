----------------------------------------------------------------
--DROP--
----------------------------------------------------------------

DROP TABLE ENCUESTA;
DROP TABLE ARRIENDOS_DISPONIBLES;
DROP TABLE ARRIENDO;
DROP TABLE PAGO;
DROP TABLE SEGURO;
DROP TABLE EMBARCACION;
DROP TABLE CLIENTE;
DROP TABLE ASISTENTE;
DROP TABLE USUARIO;

DROP SEQUENCE CREATE_ID_USUARIO;
DROP SEQUENCE CREATE_ID_ASISTENTE;
DROP SEQUENCE CREATE_ID_EMBARCACION;
DROP SEQUENCE CREATE_ID_SEGURO;
DROP SEQUENCE CREATE_ID_ARRIENDO;
DROP SEQUENCE CREATE_ID_PAGO;
DROP SEQUENCE CREATE_ID_ENCUESTA;
DROP SEQUENCE CREATE_ID_ARRIENDOS_D;

DROP PROCEDURE CREAR_USUARIO;
DROP PROCEDURE INICIAR_SESION;
DROP PROCEDURE CREAR_ENCUESTA;
DROP PROCEDURE CREAR_PAGO;
DROP PROCEDURE CREAR_EMBARCACION;
DROP PROCEDURE QUITAR_EMBARCACION;
DROP PROCEDURE FIN_ARRIENDO;
DROP PROCEDURE CREAR_ARRIENDO;
DROP PROCEDURE CREAR_PAGO_ARRIENDO;
DROP PROCEDURE UPDATE_PAGO;

DROP TRIGGER FORMATEAR_DATOS_CLIENTE;

DROP FUNCTION VERIFICAR_CLIENTE;
DROP FUNCTION OBTENER_CONTRASENA;

----------------------------------------------------------------
--TABLAS--
----------------------------------------------------------------

CREATE TABLE USUARIO (
ID_USUARIO      NUMBER NOT NULL, 
NOMBRE_USUARIO  VARCHAR(20) UNIQUE NOT NULL,
TIPO            VARCHAR(20),
CONTRASENIA     VARCHAR(20),
PRIMARY KEY (ID_USUARIO)
);

CREATE TABLE CLIENTE (
RUT             NUMBER NOT NULL,
ID_USUARIO      NUMBER UNIQUE,
NOMBRECLIENTE   VARCHAR(100),
DIRECCION       VARCHAR(140),
TELEFONO        NUMBER,
PRIMARY KEY (RUT),
FOREIGN KEY (ID_USUARIO) REFERENCES USUARIO(ID_USUARIO)
);

CREATE TABLE ASISTENTE (
ID_ASISTENTE    NUMBER NOT NULL, 
ID_USUARIO      NUMBER UNIQUE,
RUT_ASISTENTE   NUMBER,
NOMBREASISTENTE VARCHAR(100),
DIRECCION_ASISTENTE     VARCHAR(200),
TELEFONO_ASISTENTE        NUMBER,
PRIMARY KEY (ID_ASISTENTE),
FOREIGN KEY (ID_USUARIO) REFERENCES USUARIO(ID_USUARIO)
);

CREATE TABLE EMBARCACION (
ID_EMBARCACION  NUMBER NOT NULL,
ID_ASISTENTE    NUMBER,
TIPOEMBARCACION VARCHAR(20), 
PRECIO          NUMBER,
PATENTE         VARCHAR(20),
PRIMARY KEY (ID_EMBARCACION),
FOREIGN KEY (ID_ASISTENTE) REFERENCES ASISTENTE(ID_ASISTENTE)
);

CREATE TABLE SEGURO (
ID_SEGURO           NUMBER NOT NULL,
ID_EMBARCACION      NUMBER,
VALORSEGURO         NUMBER,
PRIMARY KEY (ID_SEGURO),
FOREIGN KEY (ID_EMBARCACION) REFERENCES EMBARCACION(ID_EMBARCACION)
);

CREATE TABLE PAGO (
ID_PAGO         NUMBER NOT NULL,
VALOR           NUMBER, 
TIPO            VARCHAR(20),
PRIMARY KEY     (ID_PAGO)
); 

CREATE TABLE ARRIENDO (
ID_ARRIENDO     NUMBER NOT NULL,
ID_ASISTENTE    NUMBER, 
ID_EMBARCACION  NUMBER,
RUT             NUMBER,
ID_SEGURO       NUMBER,
ID_PAGO         NUMBER, /*arreglar el insert*/
LUGAR_RETIRO    VARCHAR(200),
LUGAR_ENTREGA   VARCHAR(200),
FECHA_RETIRO    VARCHAR(20),
FECHA_ENTREGA   VARCHAR(20),
HORA_RETIRO     VARCHAR(20),
HORA_ENTREGA    VARCHAR(20),
VALOR           NUMBER, /*SUMA DEL BARCO + SEGURO*/ 
ESTADO          NUMBER(1,0), /*TRUE = COMO ARRENDADO & FALSE= NO ARRENDADO*/


PRIMARY KEY (ID_ARRIENDO),
FOREIGN KEY (ID_ASISTENTE)      REFERENCES ASISTENTE(ID_ASISTENTE),
FOREIGN KEY (ID_EMBARCACION)    REFERENCES EMBARCACION(ID_EMBARCACION),
FOREIGN KEY (RUT)               REFERENCES CLIENTE(RUT),
FOREIGN KEY (ID_SEGURO)         REFERENCES SEGURO(ID_SEGURO),
FOREIGN KEY (ID_PAGO)           REFERENCES PAGO(ID_PAGO)
);



CREATE TABLE ENCUESTA (
ID_ENCUESTA     NUMBER NOT NULL,
RUT             NUMBER,
ID_ARRIENDO     NUMBER, 
VALORACION      NUMBER,
COMENTARIO      VARCHAR(200),
PRIMARY KEY     (ID_ENCUESTA),
FOREIGN KEY     (RUT)           REFERENCES CLIENTE(RUT),
FOREIGN KEY     (ID_ARRIENDO)   REFERENCES ARRIENDO(ID_ARRIENDO)
);

CREATE TABLE ARRIENDOS_DISPONIBLES(
    ID_ARRIENDO_DISPONIBLES     NUMBER NOT NULL,
    ID_EMBARCACION              NUMBER,
    LUGAR_RETIRO                VARCHAR(20),
    LUGAR_ENTREGA               VARCHAR(20),
    FECHA_RETIRO                VARCHAR(20),
    FECHA_ENTREGA               VARCHAR(20),
    HORA_RETIRO                 VARCHAR(20),
    HORA_ENTREGA                VARCHAR(20),
    PRIMARY KEY(ID_ARRIENDO_DISPONIBLES),
    FOREIGN KEY (ID_EMBARCACION)            REFERENCES EMBARCACION(ID_EMBARCACION)
);

------------------------------------------------------------
-- FIN TABLAS 
------------------------------------------------------------
------------------------------------------------------------
-- INGRESAR USUARIOS
------------------------------------------------------------
INSERT INTO USUARIO(ID_USUARIO, NOMBRE_USUARIO, TIPO, CONTRASENIA) VALUES (001, 'JOSE',     'CLIENTE', '12342');
INSERT INTO USUARIO(ID_USUARIO, NOMBRE_USUARIO, TIPO, CONTRASENIA) VALUES (002, 'CAMILA',   'CLIENTE', '12345');
INSERT INTO USUARIO(ID_USUARIO, NOMBRE_USUARIO, TIPO, CONTRASENIA) VALUES (003, 'ANA',      'CLIENTE', '12323');
INSERT INTO USUARIO(ID_USUARIO, NOMBRE_USUARIO, TIPO, CONTRASENIA) VALUES (004, 'ANDRES',   'CLIENTE', '12334');

INSERT INTO USUARIO(ID_USUARIO, NOMBRE_USUARIO, TIPO, CONTRASENIA) VALUES (010, 'ANTONIA',  'ASISTENTE', '12342');
INSERT INTO USUARIO(ID_USUARIO, NOMBRE_USUARIO, TIPO, CONTRASENIA) VALUES (011, 'LUIS',     'ASISTENTE', '12345');
INSERT INTO USUARIO(ID_USUARIO, NOMBRE_USUARIO, TIPO, CONTRASENIA) VALUES (012, 'ABELARDO', 'ASISTENTE', '12323');
INSERT INTO USUARIO(ID_USUARIO, NOMBRE_USUARIO, TIPO, CONTRASENIA) VALUES (013, 'ANDREA',   'ASISTENTE', '12334');
INSERT INTO USUARIO(ID_USUARIO, NOMBRE_USUARIO, TIPO, CONTRASENIA) VALUES (014, 'NICOLAS',  'ASISTENTE', '12334');

INSERT INTO USUARIO(ID_USUARIO, NOMBRE_USUARIO, TIPO, CONTRASENIA) VALUES (111, 'ADMIN',  'ADMINISTRADOR', '12345');
-------------------------------------------------------------
---INSERTAR CLIENTE
-------------------------------------------------------------
INSERT INTO CLIENTE(RUT, ID_USUARIO, NOMBRECLIENTE, DIRECCION,TELEFONO) VALUES(7952209, 001,    'JOSE ROJAS VERGARA',    'FUNDO SAN FRANCISO PERQUIN',  945412330);
INSERT INTO CLIENTE(RUT, ID_USUARIO, NOMBRECLIENTE, DIRECCION,TELEFONO) VALUES(19472440, 002,   'CAMILA SEPULVEDA ROCO', 'CAMINO LO CASTILLO SN',       974145782);
INSERT INTO CLIENTE(RUT, ID_USUARIO, NOMBRECLIENTE, DIRECCION,TELEFONO) VALUES(11208575, 003,   'ANA ABARZUA ROJAS',     'EL ROBLE 1070',               965512478);
INSERT INTO CLIENTE(RUT, ID_USUARIO, NOMBRECLIENTE, DIRECCION,TELEFONO) VALUES(17091192, 004,   'ANDRES ASTORGA RUIZ',   'RANCAGUA 3024',               956845687);
               
-------------------------------------------------------------
---INSERTAR ASISTENTE 
-------------------------------------------------------------
INSERT INTO ASISTENTE(ID_ASISTENTE, ID_USUARIO, RUT_ASISTENTE, NOMBREASISTENTE, DIRECCION_ASISTENTE, TELEFONO_ASISTENTE) VALUES(010, 010, 18456258, 'ANTONIA DIAZ VERGARA',       'CAROLINA RABAT 900', 945020245);
INSERT INTO ASISTENTE(ID_ASISTENTE, ID_USUARIO, RUT_ASISTENTE, NOMBREASISTENTE, DIRECCION_ASISTENTE, TELEFONO_ASISTENTE) VALUES(011, 011, 13302521, 'LUIS JORQUERA CANALES',      'SAN MARTIN 0775'   , 984154607);
INSERT INTO ASISTENTE(ID_ASISTENTE, ID_USUARIO, RUT_ASISTENTE, NOMBREASISTENTE, DIRECCION_ASISTENTE, TELEFONO_ASISTENTE) VALUES(012, 012, 14359599, 'ABELARDO FUENTES FUENTES',   'VENEZUELA 5901'    , 987541442);
INSERT INTO ASISTENTE(ID_ASISTENTE, ID_USUARIO, RUT_ASISTENTE, NOMBREASISTENTE, DIRECCION_ASISTENTE, TELEFONO_ASISTENTE) VALUES(013, 013, 14031260, 'ANDREA MONTALBA GONZALEZ',   'RIO QUIAPO 16'     , 965511478);
INSERT INTO ASISTENTE(ID_ASISTENTE, ID_USUARIO, RUT_ASISTENTE, NOMBREASISTENTE, DIRECCION_ASISTENTE, TELEFONO_ASISTENTE) VALUES(014, 014, 19345941, 'NICOLAS COFRE BASTIAS',      'PJE VEGA ANCOA 835', 968258128);

------------------------------------------------------------
-- INGRESAR BARCOS
------------------------------------------------------------

INSERT INTO EMBARCACION(ID_EMBARCACION, ID_ASISTENTE, TIPOEMBARCACION, PRECIO, PATENTE) VALUES(100, 010,'YATE', 1000000, 'DPTH78');
INSERT INTO EMBARCACION(ID_EMBARCACION, ID_ASISTENTE, TIPOEMBARCACION, PRECIO, PATENTE) VALUES(101, 011,'YATE', 1000000, 'DQXY25');
INSERT INTO EMBARCACION(ID_EMBARCACION, ID_ASISTENTE, TIPOEMBARCACION, PRECIO, PATENTE) VALUES(102, 012,'BARCO', 500000, 'TYSJ74');
INSERT INTO EMBARCACION(ID_EMBARCACION, ID_ASISTENTE, TIPOEMBARCACION, PRECIO, PATENTE) VALUES(103, 013,'BARCO', 500000, 'TFJK20');

-------------------------------------------------------------
---INSERTAR SEGURO
-------------------------------------------------------------
INSERT INTO SEGURO(ID_SEGURO, ID_EMBARCACION, VALORSEGURO) VALUES(021, 100, 100000);
INSERT INTO SEGURO(ID_SEGURO, ID_EMBARCACION, VALORSEGURO) VALUES(022, 101, 100000);
INSERT INTO SEGURO(ID_SEGURO, ID_EMBARCACION, VALORSEGURO) VALUES(023, 102, 150000);
INSERT INTO SEGURO(ID_SEGURO, ID_EMBARCACION, VALORSEGURO) VALUES(024, 103, 150000);

-------------------------------------------------------------
---INSERTAR ARRIENDO
-------------------------------------------------------------
/*INSERT INTO ARRIENDO(ID_ARRIENDO, ID_ASISTENTE, ID_EMBARCACION, RUT, ID_SEGURO, LUGAR_RETIRO, LUGAR_ENTREGA,VALOR,ESTADO,FECHA_RETIRO,FECHA_ENTREGA,HORA_RETIRO, HORA_ENTREGA) 
VALUES(1000, 010, 100, 7952209, 021, 'CONSTITUCION', 'CONSTITUCION',1100000,0,'2021/01/16', '2021/01/16', '12:00:00', '20:00:00');

INSERT INTO ARRIENDO(ID_ARRIENDO, ID_ASISTENTE, ID_EMBARCACION, RUT, ID_SEGURO, LUGAR_RETIRO, LUGAR_ENTREGA,VALOR,ESTADO,FECHA_RETIRO,FECHA_ENTREGA,HORA_RETIRO, HORA_ENTREGA)
VALUES(1001, 011, 101, 19472440, 022, 'VALPARAISO', 'VALPARAISO',11000000,0, '2021/01/17', '2021/01/17', '11:00:00', '18:00:00');

INSERT INTO ARRIENDO(ID_ARRIENDO, ID_ASISTENTE, ID_EMBARCACION, RUT, ID_SEGURO, LUGAR_RETIRO, LUGAR_ENTREGA,VALOR,ESTADO,FECHA_RETIRO,FECHA_ENTREGA,HORA_RETIRO, HORA_ENTREGA) 
VALUES(1002, 012, 102, 11208575, 023, 'BAHIA INGLESA', 'BAHIA INGLESA',580000,0,'2021/01/18', '2021/01/18', '10:00:00', '17:00:00');

INSERT INTO ARRIENDO(ID_ARRIENDO, ID_ASISTENTE, ID_EMBARCACION, RUT, ID_SEGURO, LUGAR_RETIRO, LUGAR_ENTREGA,VALOR,ESTADO,FECHA_RETIRO,FECHA_ENTREGA,HORA_RETIRO, HORA_ENTREGA) 
VALUES(1003, 013, 103, 17091192, 024, 'SAN ANTONIO', 'SAN ANTONIO',580000,0, '2021/01/19', '2021/01/19', '09:00:00','16:00:00');

-------------------------------------------------------------
---INSERTAR PAGO
-------------------------------------------------------------
INSERT INTO PAGO(ID_PAGO,ID_ARRIENDO,VALOR,TIPO) VALUES(10000,1100000,'YATE');
INSERT INTO PAGO(ID_PAGO,ID_ARRIENDO,VALOR,TIPO) VALUES(10001,1100000,'YATE');
INSERT INTO PAGO(ID_PAGO,ID_ARRIENDO,VALOR,TIPO) VALUES(10002,580000,'BARCO');
INSERT INTO PAGO(ID_PAGO,ID_ARRIENDO,VALOR,TIPO) VALUES(10003,580000,'BARCO');

-------------------------------------------------------------
---INSERTAR ENCUESTA
-------------------------------------------------------------
INSERT INTO ENCUESTA(ID_ENCUESTA,RUT,ID_ARRIENDO,VALORACION,COMENTARIO) VALUES(100000,7952209,1000,5,'GRAX POR TANTO CARO');
INSERT INTO ENCUESTA(ID_ENCUESTA,RUT,ID_ARRIENDO,VALORACION,COMENTARIO) VALUES(100001,19472440,1001,4,'HOLITO');
INSERT INTO ENCUESTA(ID_ENCUESTA,RUT,ID_ARRIENDO,VALORACION,COMENTARIO) VALUES(100002,11208575,1002,3,'BONITO EL BARCO');
INSERT INTO ENCUESTA(ID_ENCUESTA,RUT,ID_ARRIENDO,VALORACION,COMENTARIO) VALUES(100003,17091192,1003,2,'PESIMO SERVICIO');*/

-------------------------------------------------------------
---ARRIENDOS DISPONIBLES
-------------------------------------------------------------
INSERT INTO ARRIENDOS_DISPONIBLES(ID_ARRIENDO_DISPONIBLES,ID_EMBARCACION,LUGAR_RETIRO,LUGAR_ENTREGA,FECHA_RETIRO,FECHA_ENTREGA,HORA_RETIRO,HORA_ENTREGA)
VALUES(100100,100,'CONSTITUCION','CONSTITUCION', '2021/01/16', '2021/01/16', '12:00:00', '20:00:00');
INSERT INTO ARRIENDOS_DISPONIBLES(ID_ARRIENDO_DISPONIBLES,ID_EMBARCACION,LUGAR_RETIRO,LUGAR_ENTREGA,FECHA_RETIRO,FECHA_ENTREGA,HORA_RETIRO,HORA_ENTREGA)
VALUES(100101,101,'VALPARAISO','VALPARAISO', '2021/01/20', '2021/01/20', '11:00:00', '18:00:00');
INSERT INTO ARRIENDOS_DISPONIBLES(ID_ARRIENDO_DISPONIBLES,ID_EMBARCACION,LUGAR_RETIRO,LUGAR_ENTREGA,FECHA_RETIRO,FECHA_ENTREGA,HORA_RETIRO,HORA_ENTREGA)
VALUES(100102,102,'BAHÍA INGLESA','BAHÍA INGLESA', '2021/01/18', '2021/01/18', '10:00:00', '17:00:00');
INSERT INTO ARRIENDOS_DISPONIBLES(ID_ARRIENDO_DISPONIBLES,ID_EMBARCACION,LUGAR_RETIRO,LUGAR_ENTREGA,FECHA_RETIRO,FECHA_ENTREGA,HORA_RETIRO,HORA_ENTREGA)
VALUES(100103,103,'SAN ANTONIO','SAN ANTONIO', '2021/01/19', '2021/01/19', '10:00:00','17:00:00');


-------------------------------------------------------------
-- INICIO PROCEDURE
-------------------------------------------------------------

CREATE OR REPLACE PROCEDURE CREAR_USUARIO (

NOMBRE_USUARIO  IN USUARIO.NOMBRE_USUARIO%TYPE,
CONTRASENIA     IN USUARIO.CONTRASENIA%TYPE,
TIPO            IN CLIENTE.TIPO%TYPE,
RUTN            IN CLIENTE.RUT%TYPE,
NOMBRE_CLIENTE  IN CLIENTE.NOMBRECLIENTE%TYPE,
TELEFONO        IN CLIENTE.TELEFONO%TYPE, 
DIRECCION       IN CLIENTE.DIRECCION%TYPE,

--RETURN MENSAJE
MENSAJE     OUT     VARCHAR2,
RESULTADO   OUT     VARCHAR2
)
    IS
       CONSULTA NUMBER; 
       
    BEGIN
    
    SELECT COUNT(RUT) INTO CONSULTA FROM CLIENTE WHERE RUT = RUTN;
    IF CONSULTA = 0 THEN
        INSERT INTO CLIENTE VALUES(NOMBRE_USUARIO, CONTRASENIA, TIPO, RUTN, NOMBRECLIENTE, TELEFONO, DIRECCION, MENSAJE, RESULTADO);
        MENSAJE     := 'EL CLIENTE HA SIDO INGRESADO CON EXITO';
        RESULTADO   := 'TRUE';
        COMMIT;
    ELSE
         RESULTADO := 'FALSE';
         
    END IF;
        
        EXCEPTION
            WHEN DUP_VAL_ON_INDEX THEN
                MENSAJE := 'EL CLIENTE YA EXISTE EN LA BASE DE DATOS';
                RESULTADO := 'FALSE';
                ROLLBACK;
            WHEN OTHERS THEN
                RESULTADO := 'FALSE';
                DBMS_OUTPUT.PUT_LINE('SE HA PRODUCIDO UN ERROR AL REALIZAR LA ACCION');
                ROLLBACK;
        
END;
/

----------------------------------------------------
-- INICIAR SESION
----------------------------------------------------

CREATE OR REPLACE PROCEDURE INICIAR_SESION(

    RUTN        IN CLIENTE.RUT%TYPE,
    CONTRASENIA IN USUARIO.CONTRASENIA%TYPE,

    RESULTADO   OUT VARCHAR2,
    MENSAJE     OUT VARCHAR2)
IS
    CONTRASENA_ALMACENADA VARCHAR2(200);
BEGIN
    IF VERIFICAR_CLIENTE(RUTN) = TRUE THEN
       CONTRASENA_ALMACENADA := OBTENER_CONTRASENA(RUTN);
       IF CONTRASENIA = CONTRASENA_ALMACENADA THEN
            RESULTADO   :=  'TRUE';
            MENSAJE     :=  'HA INICIADO SESION EXITOSAMENTE';
       ELSE
            RESULTADO   :=  RUTN;
            MENSAJE     :=  CONTRASENIA;
        END IF;
    ELSE
        RESULTADO   :=  'FALSE';
        MENSAJE     :=  'LOS DATOS INGRESADOS SON INCORRECTOS';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
            RESULTADO   :=  'FALSE';
            MENSAJE     :=  'NO SE HA  INICIADO SESION';
END;
/

----------------------------------------------------
-- VERIFICAR SESION
----------------------------------------------------

CREATE OR REPLACE FUNCTION VERIFICAR_CLIENTE(
    RUT_C VARCHAR2
    )
    RETURN BOOLEAN
IS
    EXISTEC NUMBER;
BEGIN
    SELECT COUNT(*) INTO EXISTEC FROM CLIENTE WHERE RUT_C = RUT;
    IF EXISTEC > 0 THEN
        RETURN TRUE;
    ELSE
        RETURN FALSE;
    END IF;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RETURN FALSE;
        WHEN OTHERS THEN
            RETURN FALSE;
END;
/

----------------------------------------------------
-- CONSULTAR CONTRASEÑA
----------------------------------------------------
CREATE OR REPLACE FUNCTION OBTENER_CONTRASENA(
    RUT_CLIENTE NUMBER
    )
    RETURN VARCHAR2
IS
    EXISTEC NUMBER;
    CLAVEP VARCHAR2(200);
BEGIN
    SELECT CONTRASENIA INTO CLAVEP FROM CLIENTE WHERE RUT_CLIENTE = RUT;
    DBMS_OUTPUT.PUT_LINE(CLAVEP);
    IF CLAVEP != 'NO' THEN
        RETURN CLAVEP;
    ELSE
        RETURN 'NO';
    END IF;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RETURN 'NO';
        WHEN OTHERS THEN
            RETURN 'NO';
END;
/



-------------------------
--ENCUESTA
-------------------------
CREATE OR REPLACE PROCEDURE CREAR_ENCUESTA (
    E_RUT           IN CLIENTE.RUT%TYPE,
    E_ID_ARRIENDO   IN ARRIENDO.ID_ARRIENDO%TYPE,
    E_VALORACION    IN ENCUESTA.VALORACION%TYPE,
    E_COMENTARIO    IN ENCUESTA.COMENTARIO%TYPE 
    RESULTADO   OUT VARCHAR2,
    MENSAJE     OUT VARCHAR2
)
IS
    NOEXISTECLIENTE     EXCEPTION;
    NOEXISTEARRIENDO    EXCEPTION;
    EXISTECLIENTE       NUMBER;
    EXISTEARRIENDO      NUMBER;
BEGIN

	LOCK TABLE ENCUESTA IN ROW EXCLUSIVE MODE;
    /*VALIDAR QUE EL USUARIO Y EL ARRIENDO EXISTA ENTONCES SE PUEDE INGRESAR UNA ENCUESTA*/ 
    SELECT COUNT(*) INTO EXISTECLIENTE FROM CLIENTES WHERE RUT = E_RUT;
    SELECT COUNT(*) INTO EXISTEARRIENDO FROM ARRIENDO WHERE ID_ARRIENDO = E_ID_ARRIENDO; 
    /*SI NO EXISTEN UN CLIENTE Y UN ARRIENDO NO SE PUEDE HACER LA ENCUESTA*/
    IF (EXISTECLIENTE > 0) THEN 
        IF (EXISTEARRIENDO > 0) THEN
            INSERT INTO ENCUESTA VALUES( E_RUT, E_ID_ARRIENDO, E_VALORACION, E_COMENTARIO, MENSAJE, RESULTADO);
            COMMIT;
            RESULTADO   :=  'TRUE';
            MENSAJE     :=  'SE HA CREADO CON EXITO';
        ELSE
            RAISE NOEXISTEARRIENDO;
        END IF;
    ELSE
        RAISE NOEXISTECLIENTE;
    END IF;

    EXCEPTION
        WHEN NOEXISTECLIENTE THEN
            RESULTADO   :=  'FALSE';
            MENSAJE     :=  'NO SE HA CREADO CON EXITO';
			DBMS_OUTPUT.PUT_LINE('NO HAY UN CLIENTE');
			ROLLBACK;
        WHEN NOEXISTEARRIENDO THEN
            RESULTADO   :=  'FALSE';
            MENSAJE     :=  'NO SE HA CREADO CON EXITO';
			DBMS_OUTPUT.PUT_LINE('NO EXISTE UN ARRIENDO');
			ROLLBACK;          
		WHEN OTHERS THEN
            RESULTADO   :=  'FALSE';
            MENSAJE     :=  'NO SE HA CREADO CON EXITO';
			DBMS_OUTPUT.PUT_LINE(SQLCODE ||''|| SQLERRM);
			ROLLBACK;
END;
/

-----------------------------
--PARA HACE UN PAGO
-----------------------------
CREATE OR REPLACE PROCEDURE CREAR_PAGO (
    H_VALOR         IN PAGO.VALOR%TYPE,
    H_TIPO          IN PAGO.TIPO%TYPE,

    RESULTADO   OUT VARCHAR2,
    MENSAJE     OUT VARCHAR2
)
IS
    NOEXISTE_ARRIENDO   EXCEPTION;
    EXISTE_ARRIENDO     NUMBER;
BEGIN

    SELECT COUNT(*) INTO EXISTE_ARRIENDO FROM ARRIENDO WHERE ID_ARRIENDO = H_ID_ARRIENDO;
    IF (EXISTE_ARRIENDO < 0) THEN
        INSERT INTO PAGO VALUES( H_VALOR, H_TIPO, MENSAJE, RESULTADO);
        COMMIT;
        RESULTADO   :=  'TRUE';
        MENSAJE     :=  'SE HA HECHO EL PAGO CORRECTAMENTE';
       
    ELSE
        RAISE NOEXISTE_ARRIENDO;
    END IF;

    EXCEPTION
    WHEN NOEXISTE_ARRIENDO THEN

        RESULTADO   :=  'FALSE';
        MENSAJE     :=  'NO SE HA HECHO UN PAGO';


		DBMS_OUTPUT.PUT_LINE('NO SE HA HECHO UN PAGO');
		ROLLBACK;
END;
/

-----------------------------------------------
--CREAR PAGO ARRIENDO
-----------------------------------------------
CREATE OR REPLACE PROCEDURE CREAR_PAGO_ARRIENDO (
    H_ID_EMBARACION         IN EMBARCACION.ID_EMBARCACION%TYPE,
    H_TIPO                  IN PAGO.TIPO%TYPE,
    RESULTADO   OUT VARCHAR2,
    MENSAJE     OUT NUMBER
)
AS
    NOEXISTE_EMBARCACION   EXCEPTION;
    PRECIO_EMBARCACION NUMBER;
    PRECIO_SEGURO NUMBER;
    NUEVO_ID NUMBER;
BEGIN

    SELECT PRECIO INTO PRECIO_EMBARCACION FROM EMBARCACION WHERE ID_EMBARCACION = H_ID_EMBARACION;
    IF (PRECIO_EMBARCACION > 0) THEN
        SELECT VALORSEGURO INTO PRECIO_SEGURO FROM SEGURO WHERE ID_EMBARCACION = H_ID_EMBARACION;
        NUEVO_ID := CREATE_ID_PAGO.NEXTVAL;
        INSERT INTO PAGO(ID_PAGO,VALOR,TIPO) VALUES(NUEVO_ID,(PRECIO_SEGURO + PRECIO_EMBARCACION),H_TIPO);
        COMMIT;
        RESULTADO   :=  'TRUE';
        MENSAJE     :=  NUEVO_ID;

    ELSE
        RAISE NOEXISTE_EMBARCACION;
    END IF;

    EXCEPTION
    WHEN NOEXISTE_EMBARCACION THEN

        RESULTADO   :=  'FALSE';
        MENSAJE     :=  'NO SE HA HECHO UN PAGO';


		DBMS_OUTPUT.PUT_LINE('NO SE HA HECHO UN PAGO');
		ROLLBACK;
END;
/

----------------------------------------------------
-- INSERTAR BARCO
----------------------------------------------------
CREATE OR REPLACE PROCEDURE CREAR_EMBARCACION(
IDASISTENTE     IN EMBARCACION.ID_ASISTENTE%TYPE,
TIPOEMB         IN EMBARCACION.TIPOEMBARCACION%TYPE,
PRECIO          IN EMBARCACION.PRECIO%TYPE,
INGRESAPATENTE  IN EMBARCACION.PATENTE%TYPE,

---RETURN MENSAJE
    MENSAJE     OUT     VARCHAR2,
    RESULTADO   OUT     VARCHAR2
)

    IS 
    DECLARE @IDBARCO_ NUMBER;

    IDBARCO_ES_ NUMBER NOT NULL; 
        EXISTE BOOLEAN; 
        CONSTRAINT IDBARCO_ES_ := IDBARCO
            CHECK (IDBARCO_ES_ > 0);

--CURSOR PARA VER LOS ID DE LOS BARCOS EXISTENTES
DECLARE CURSOR CURSOR_EMB
IS 
    SELECT ID_EMBARCACION
    FROM EMBARCACION
    BEGIN
        OPEN CURSOR_EMB;
        FETCH CURSOR_EMB INTO @IDBARCO_
        IF IDBARCO_ES_ != @IDBARCO_
            THEN INSERT INTO EMBARCACION VALUES (IDASISTENTE, TIPOEMB,PRECIO, INGRESAPRECIO, INGRESAPATENTE, MENSAJE, RESULTADO);
            COMMIT;
            RESULTADO   :=  'TRUE';
            MENSAJE     :=  'SE HA INGRESADO EXITOSAMENTE';
        ELSE
            MENSAJE := "ESTA EMBARCACION YA EXISTE";
            RESULTADO   :=  'FALSE';
            DBMS_OUTPUT.PUT_LINE(''||MENSAJE)
    EXCEPTION
        WHEN OTHERS THEN 
            RESULTADO   :=  'TRUE';
            MENSAJE := "EL BARCO HA SIDO INGRESADO CON EXITO"; /*MUESTRA MENSAJE DEL SQL*/
            DBMS_OUTPUT.PUT_LINE(''||MENSAJE);
            ROLLBACK;
END;
/
----------------------------------------------------
-- BORRAR BARCO 
----------------------------------------------------
CREATE OR REPLACE PROCEDURE QUITAR_EMBARCACION(

    ID_EMBARCACION IN EMBARCACION.ID_EMBARCACION%TYPE,
---RETURN MENSAJE     
    RESULTADO   OUT VARCHAR2,
    MENSAJE     OUT VARCHAR2
)
-----------------BORRADO DE TIPO 
    AS
    BEGIN
        BEGIN TRY
            BEGIN TRANSACTION;
            RESULTADO := 'TRUE';
            MENSAJE := 'SE HA ELIMINADO CON EXITO';
            delete from embarcacion where EMBARCACION.ID_EMBARCACION = :idEmbarcacion;
            COMMIT TRANSACTION; 
        END TRY

        BEGIN CATCH
            EXEC usp_report_error;
            IF (XACT_STATE()) = -1
            BEGIN 
                SET MENSAJE = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO NO COMPROMETIDO.';
                ROLLBACK TRANSACTION;
                RESULTADO := 'FALSE';
                MENSAJE := 'NO SE HA ELIMINADO CON EXITO';
            END;
        
            IF (XACT_STATE()) = 1
            BEGIN
                RESULTADO := 'FALSE';
                SET MENSAJE = 'ESTA ACCION NO SE ENCUENTRA COMPROMETIDA';
                COMMIT TRANSACTION;
            END;
        END CATCH
    END; 
/


----------------------------------------------------
-- FINALIZACION ARRIENDO 
----------------------------------------------------
CREATE OR REPLACE PROCEDURE FIN_ARRIENDO(
    ID_ARRIENDO_ES  IN ARRIENDO.ID_ARRIENDO%TYPE, 
    ESTADO_ES       IN ARRIENDO.ESTADO%TYPE,
--OUT 
    MENSAJE         OUT VARCHAR2, 
    RESULTADO       OUT VARCHAR2,
    ESTADO_ES       OUT BOOLEAN

    )
        IS 
            DECLARE @ID_ARRIENDO_ES NUMBER
                    @ESTADO_ES BOOLEAN

        IDARRIENDO_ESTADO NUMBER NOT NULL;
        ES_ESTADO BOOLEAN; 
            CONSTRAINT IDARRIENDO_ESTADO := ID_ARRIENDO_ES
            CHECK (IDARRIENDO_ESTADO > 0); 
            CONSTRAINT ES_ESTADO :=  ESTADO_ES;

        DECLARE CURSOR CURSOR_ESTADO 
        IS 
            SELECT ESTADO
            FROM ARRIENDO 
            WHEN ID_ARRIENDO = IDARRIENDO_ESTADO    
        
        BEGIN 
            OPEN CURSOR_ESTADO;
            FLETCH CURSOR_ESTADO INTO @ESTADO_ES; 

            IF ES_ESTADO = @ESTADO_ES THEN 
                IF @ESTADO_ES = TRUE THEN 
                    RESULTADO   :=  'FALSE';
                    MENSAJE := "LA EMBARCACIÓN SE ENCUENTRA EN ARRIENDO"
                    DBMS_OUTPUT.PUT_LINE(''||MENSAJE);
                ELSE 
                    RESULTADO   :=  'TRUE';
                    MENSAJE := "LA EMBARCACIÓN HA FINALIZADO"
                    DBMS_OUTPUT.PUT_LINE(''||MENSAJE); 
            ELSE 
                RESULTADO   :=  'TRUE';
                INSERT INTO ARRIENDO(ESTADO) VALUES ES_ESTADO;
                COMMIT;
        EXCEPTION
            WHEN OTHERS THEN 
                RESULTADO   :=  'TRUE';
                MENSAJE := "EL ESTADO DEL BARCO A SIDO MODIFICADO"
                IF ES_ESTADO := TRUE THEN MENSAJE :="LA EMBARCACIÓN ESTÁ EN ARRIENDO"
                DBMS_OUTPUT.PUT_LINE(''||MENSAJE);
                ELSE 
                    RESULTADO   :=  'FALSE';
                    MENSAJE := "LA EMBARCACIÓN AHORA SE ENCUENTRA DISPONIBLE"
                    DBMS_OUTPUT.PUT_LINE(''||MENSAJE);    
       
        CLOSE CURSOR_ESTADO;
        END; 
        /


----------------------------------------------------------------
--USUARIOS--
----------------------------------------------------------------
--ADMINISTRADOR 
----------------------------------------------------
CREATE USER administrador IDENTIFIED BY user;
    GRANT all privileges to administrador; 

CREATE user admin IDENTIFIED by lock;
     GRANT create session to admin;
        GRANT execute   on CREAR_EMBARCACION   to admin; /*proceso de ingreso de dato*/
        GRANT execute   on QUITAR_EMBARCACION     to admin;
        GRANT SELECT    on ARRIENDO               to admin;
        GRANT SELECT    on CLIENTE                to admin; 
        GRANT SELECT    on ENCUESTA               to admin; 
        GRANT select    on ASISTENTE              to admin;
        GRANT SELECT    on EMBARCACION            to admin; 

--CLIENTE
----------------------------------------------------
CREATE user josecito IDENTIFIED by lock; /*VISTA DEL CLIENTE*/
    GRANT CREATE SESSION TO josecito;
    GRANT execute       ON INICIAR_SESION         TO josecito; 
    GRANT SELECT        ON ARRIENDO               TO josecito;
    GRANT SELECT        ON ENCUESTA               TO josecito; /*excute administrador.PROCESO DE LA ENCUESTA!!!!*/
    GRANT SELECT        ON SEGURO                 TO josecito; 
    GRANT SELECT        ON CLIENTE                TO josecito;

--CLIENTE
----------------------------------------------------
CREATE user asist IDENTIFIED by 1234; /*VISTA DEL ASISTENTE*/
    GRANT CREATE SESSION TO asist;          /*EL ASISTENTE DEBE TENER EJECUTAR LA FUNCION DE CAMBIO DE ESTADO*/
    GRANT execute       ON FIN_ARRIENDO           TO asist;
    GRANT SELECT        ON  ARRIENDO              TO asist; 
    GRANT SELECT        ON ENCUESTA               TO asist;
    GRANT SELECT        ON EMBARCACION            TO asist; 
    GRANT SELECT        ON CLIENTE                TO asist;
    GRANT SELECT        ON ASISTENTE              TO asist;


CREATE OR REPLACE PROCEDURE CREAR_ARRIENDO (
    NEW_RUT                     IN CLIENTE.RUT%TYPE,
    NEW_ID_EMBARCACION          IN EMBARCACION.ID_EMBARCACION%TYPE,
    NEW_ID_ARRIENDO_DISPONIBLES IN ARRIENDOS_DISPONIBLES.ID_ARRIENDO_DISPONIBLES%TYPE,
    NEW_ID_PAGO                 IN PAGO.ID_PAGO%TYPE,

    RESULTADO   OUT VARCHAR2,
    MENSAJE     OUT VARCHAR2
)
IS
    EXISTE_CLIENTE                  NUMBER;
    NOEXISTE_CLIENTE                EXCEPTION;

    EXISTE_EMBARACACION             NUMBER;
    NOEXISTE_EMBARCACION            EXCEPTION;

    EXISTE_ARRIENDOS_DISPONIBLES    NUMBER;
    NOEXISTE_ARRIENDOS_DISPONIBLES  EXCEPTION;

    BUSCAR_SEGURO                   NUMBER;
    PRECIO_EMBARCACION              NUMBER;
    VALORSEGURO                     NUMBER;
	NUEVO_ID                        NUMBER;
    PRECIO_SEGURO                   NUMBER;
    BUSCAR_ASISTENTE                NUMBER;
    /*AQUI SE VAN A ALMACENAR TODOS LOS VALORES QUE CONTIENE UN REGISTRO DE LA TABLA ARRIENDOS DISPONIBLES*/
    BUSCAR_ARRIENDO_DISPONIBLE ARRIENDOS_DISPONIBLES%ROWTYPE;

BEGIN

    SELECT COUNT(*) INTO EXISTE_CLIENTE                 FROM CLIENTE           WHERE RUT                     = NEW_RUT;
    SELECT COUNT(*) INTO EXISTE_EMBARACACION            FROM EMBARCACION       WHERE ID_EMBARCACION          = NEW_ID_EMBARCACION;
    SELECT COUNT(*) INTO EXISTE_ARRIENDOS_DISPONIBLES   FROM ARRIENDOS_DISPONIBLES WHERE ID_ARRIENDO_DISPONIBLES = NEW_ID_ARRIENDO_DISPONIBLES;
    RESULTADO := 'FALSE';

    IF (EXISTE_CLIENTE > 0) THEN
        IF (EXISTE_EMBARACACION > 0) THEN
            IF (EXISTE_ARRIENDOS_DISPONIBLES > 0) THEN
                    /*SE BUSCA EL ID DEL SEGURO*/
                    SELECT ID_SEGURO, VALORSEGURO INTO BUSCAR_SEGURO, PRECIO_SEGURO FROM SEGURO WHERE ID_EMBARCACION = NEW_ID_EMBARCACION;
                    SELECT * INTO BUSCAR_ARRIENDO_DISPONIBLE FROM ARRIENDOS_DISPONIBLES WHERE ID_ARRIENDO_DISPONIBLES = NEW_ID_ARRIENDO_DISPONIBLES;
                    SELECT PRECIO, ID_ASISTENTE INTO PRECIO_EMBARCACION, BUSCAR_ASISTENTE FROM EMBARCACION WHERE ID_EMBARCACION = NEW_ID_EMBARCACION;
                    NUEVO_ID := CREATE_ID_ARRIENDO.NEXTVAL;
					RESULTADO   :=  'TRUE';
                    MENSAJE     :=  NUEVO_ID;

                    INSERT INTO ARRIENDO VALUES(NUEVO_ID, BUSCAR_ASISTENTE, NEW_ID_EMBARCACION, NEW_RUT, BUSCAR_SEGURO, NEW_ID_PAGO,
                                              BUSCAR_ARRIENDO_DISPONIBLE.LUGAR_RETIRO, BUSCAR_ARRIENDO_DISPONIBLE.LUGAR_ENTREGA,
                                              BUSCAR_ARRIENDO_DISPONIBLE.FECHA_RETIRO, BUSCAR_ARRIENDO_DISPONIBLE.FECHA_ENTREGA,
                                              BUSCAR_ARRIENDO_DISPONIBLE.HORA_RETIRO, BUSCAR_ARRIENDO_DISPONIBLE.HORA_ENTREGA,
                                              PRECIO_EMBARCACION+PRECIO_SEGURO, 0);

                    /*HACER QUE EL ARRIENDO DEJE DE ESTAR DISPONIBLE PORQUE FUE USADO*/
                    DELETE FROM ARRIENDOS_DISPONIBLES WHERE ID_ARRIENDO_DISPONIBLES = NEW_ID_ARRIENDO_DISPONIBLES;

                    COMMIT;
                    RESULTADO := 'TRUE';
            ELSE
                RAISE NOEXISTE_ARRIENDOS_DISPONIBLES;
            END IF;
        ELSE
            RAISE NOEXISTE_EMBARCACION;
        END IF;
    ELSE
        RAISE NOEXISTE_CLIENTE;
    END IF;

    EXCEPTION
    WHEN NOEXISTE_CLIENTE THEN
        RESULTADO   :=  'FALSE';
        MENSAJE     :=  'NO SE HA CREADO CON EXITO EL CLIENTE';
		DBMS_OUTPUT.PUT_LINE('NO SE ENCUENTRA REGISTRADO EL CLIENTE');
		ROLLBACK;

    WHEN NOEXISTE_EMBARCACION THEN
        RESULTADO   :=  'FALSE';
        MENSAJE     :=  'NO SE HA CREADO CON EXITO LA EMBARCACION';
		DBMS_OUTPUT.PUT_LINE('NO SE ENCUENTRA SELECCIONADA LA EMBARCACION');
		ROLLBACK;

    WHEN NOEXISTE_ARRIENDOS_DISPONIBLES THEN
        RESULTADO   :=  'FALSE';
        MENSAJE     :=  'NO SE HA CREADO CON EXITO EL ARRIENDO';
		DBMS_OUTPUT.PUT_LINE('NO HAY ARRIENDOS DISPONIBLES');
		ROLLBACK;


END;
/

-------------------------------------------------------------------------
-- CREACION DE ID INCREMENTADO
------------------------------------------------------------------------

CREATE SEQUENCE Create_id_usuario
MINVALUE 1
START WITH 1
INCREMENT BY 1
CACHE 10; 

CREATE SEQUENCE Create_id_asistente
MINVALUE 1
START WITH 1
INCREMENT BY 1
CACHE 10; 

CREATE SEQUENCE Create_id_embarcacion
MINVALUE 1
START WITH 1
INCREMENT BY 1
CACHE 10; 

CREATE SEQUENCE Create_id_seguro
MINVALUE 1
START WITH 1
INCREMENT BY 1
CACHE 10; 

CREATE SEQUENCE Create_id_arriendo
MINVALUE 1
START WITH 1
INCREMENT BY 1
CACHE 10; 

CREATE SEQUENCE Create_id_pago
MINVALUE 1
START WITH 1
INCREMENT BY 1
CACHE 10; 

CREATE SEQUENCE Create_id_encuesta
MINVALUE 1
START WITH 1
INCREMENT BY 1
CACHE 10; 

CREATE SEQUENCE CREATE_ID_ARRIENDOS_D
MINVALUE 1
START WITH 1
INCREMENT BY 1
CACHE 10; 

-------------------------------------------------------------------------
-- trigger
------------------------------------------------------------------------
CREATE OR REPLACE TRIGGER FORMATEAR_DATOS_CLIENTE
BEFORE INSERT ON CLIENTE
FOR EACH ROW
DECLARE 
    RUT_INVALIDO EXCEPTION;
    CLIENTE_DUPLICADO EXCEPTION;
    CONTADOR NUMBER;

BEGIN
    IF VALIDACION(:NEW.RUT) = TRUE THEN
    SELECT COUNT(*) INTO CONTADOR FROM CLIENTE WHERE RUT= :NEW.RUT;
        IF CONTADOR = 0 THEN
            :ID_USUARIO     := UPPER(NEW.ID_USUARIO);
            :NOMBRECLIENTE  := UPPER(NEW.NOMBRECLIENTE)
            :NEW.DIRECCION  := UPPER(NEW.DIRECCION);
            :TELEFONO       := UPPER(NEW.TELEFONO);
        ELSE
            RAISE CLIENTE_DUPLICADO;
            END IF;
    ELSE 
        RAISE RUT_INVALIDO;
    END IF;
    EXCEPTION
        WHEN RUT_INVALIDO THEN
            RAISE_APPLICATION_ERROR(-20001, 'RUT INVALIDO EN TRIGGER');
        WHEN CLIENTE_DUPLICADO THEN
            RAISE_APPLICATION_ERROR(-20001, 'CLIENTE DUPLICADO EN TABLA CLIENTE');
    END;
    /

----------------------------------------------------
-- INSERTAR CLIENTE
----------------------------------------------------
CREATE OR REPLACE PROCEDURE CREAR_CLIENTE(
	RUTCLIENTE       IN CLIENTE.RUT%TYPE,
	IDUSERCLIENTE    IN CLIENTE.ID_USUARIO%TYPE,
	NOMBRECLIENT     IN CLIENTE.NOMBRECLIENTE%TYPE,
	DIRECCIONCLIENTE IN CLIENTE.DIRECCION%TYPE,
	TELEFONOCLIENTE  IN CLIENTE.TELEFONO%TYPE,
	--RETURN RESULTADO 
	MENSAJECLI     OUT     VARCHAR2,
    RESULTADOCLI   OUT     VARCHAR2
)

	IS 	
	DECLARE RUTCLIENT NUMBER;

	AUX_RUTCLIENTE NUMBER NOT NULL; 
		CONSTRAINT AUX_RUTCLIENTE := RUTCLIENTE
			CHECK (AUX_RUTCLIENTE > 0):

	--CURSOR PARA CORROBORAR EL RUT DE LOS CLIENTES EXISTENTES 
	DECLARE CURSOR CURSOR_CLIENTE
	IS 
		SELECT RUT 
		FROM CLIENTE
		BEGIN 
			OPEN CURSOR_CLIENTE;
			FETCH CURSOR_CLIENTE INTO RUTCLIENT
			IF AUX_RUTCLIENTE != RUTCLIENT
				THEN INSERT INTO CLIENTE VALUES(AUX_RUTCLIENTE, IDUSERCLIENTE, NOMBRECLIENT, DIRECCIONCLIENTE, TELEFONOCLIENTE);
				COMMIT; 
				RESULTADOCLI := 'TRUE';
				MENSAJECLI   := 'SE HA INGRESADO EXITOSAMENTE';
			ELSE
				MENSAJECLI   := 'ESTE CLIENTE YA EXISTE'
				RESULTADOCLI    :=  'FALSE';
				DBMS_OUTPUT.PUT_LINE(''|| MENSAJECLI)
		EXCEPTION
			WHEN OTHERS THEN 
				RESULTADOCLI := 'TRUE';
				MENSAJECLI   := 'HA SIDO INGRESADO CON EXITO';
				DBMS_OUTPUT.PUT_LINE(''|| MENSAJECLI)
				ROLLBACK; 
END; 
/

----------------------------------------------------
-- BORRAR CLIENTE
----------------------------------------------------
CREATE OR REPLACE PROCEDURE QUITAR_CLIENTE(
	RUT_CLIENTE IN CLIENTE.RUT%TYPE,

	---RETURN
	RESULTADOCLIENTE  OUT VARCHAR2,
	MENSAJECLIENTE    OUT VARCHAR2
)

	IS DECLARE RUTCLI     NUMBER, 
			   ID_CLIENTE NUMBER,
			   NOMBRECLI  VARCHAR2,
			   DIRECCLI   VARCHAR2,
			   TELCLI     NUMBER

	--CURSOR PARA ENCONTRAR LOS VALORES APARTIR DEL RUT DEL CLIENTE
	DECLARE CURSOR CURSOR_BORROCLIENTE
	IS 
		SELECT *
		FROM CLIENTE 
		WHEN RUT = RUT_CLIENTE
		BEGIN 
			OPEN CURSOR_BORROCLIENTE;
			FETCH CURSOR_BORROCLIENTE INTO RUTCLI, ID_CLIENTE, NOMBRECLI, DIRECCLI, TELCLI 

-----------------BORRADO DE NOMBRE
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
			DELETE FROM CLIENTE WHERE NOMBRECLIENTE = NOMBRECLI;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJECLIENTE = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJECLIENTE = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO


-----------------BORRADO DE DIRECCION
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
			DELETE FROM CLIENTE WHERE DIRECCION = DIRECCLI;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJECLIENTE = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJECLIENTE = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO DE DIRECCION
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
			DELETE FROM CLIENTE WHERE TELEFONO = TELCLI;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJECLIENTE = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJECLIENTE = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO
/

----------------------------------------------------
-- EDITAR CLIENTE
----------------------------------------------------
--
--
--
---
--
--
--
--
--
--
--
--


----------------------------------------------------
-- INSERTAR ASISTENTE
----------------------------------------------------
CREATE OR REPLACE PROCEDURE CREAR_CLIENTE(
	IDENTASISTENTE      IN ASISTENTE.ID_ASISTENTE%TYPE,
	IDUSERASIST         IN ASISTENTE.ID_USUARIO%TYPE,
	RUTASIST            IN ASISTENTE.RUT%TYPE,
	NOMBREASISTENTE     IN ASISTENTE.NOMBREASISTENTE%TYPE,
	DIRECCIONASISTENTE  IN ASISTENTE.DIRECCION%TYPE,
	TELEFONOASISTENTE   IN ASISTENTE.TELEFONO%TYPE,
	--RETURN RESULTADO 
	MENSAJEASIST     OUT     VARCHAR2,
    RESULTADOASIST   OUT     VARCHAR2
)

	IS 	
	DECLARE IDASIST NUMBER;

	AUX_RUTCLIENTE NUMBER NOT NULL; 
		CONSTRAINT AUX_IDASISTENT := IDENTASISTENTE
			CHECK (AUX_IDASISTENT > 0):

	--CURSOR PARA CORROBORAR EL ID DE ASISTENTES EXISTENTES 
	DECLARE CURSOR CURSOR_ASISTENTE
	IS 
		SELECT ID_ASISTENTE 
		FROM ASISTENTE
		BEGIN 
			OPEN CURSOR_ASISTENTE;
			FETCH CURSOR_ASISTENTE INTO IDASIST
			IF AUX_IDASISTENT != IDASIST
				THEN INSERT INTO ASISTENTE VALUES(AUX_IDASISTENT, IDUSERASIST, RUTASIST, NOMBREASISTENTE, DIRECCIONASISTENTE, TELEFONOASISTENTE);
				COMMIT; 
				RESULTADOASIST := 'TRUE';
				MENSAJEASIST   := 'SE HA INGRESADO EXITOSAMENTE';
			ELSE
				MENSAJEASIST   := 'ESTE CLIENTE YA EXISTE'
				RESULTADOASIST    :=  'FALSE';
				DBMS_OUTPUT.PUT_LINE(''|| MENSAJEASIST)
		EXCEPTION
			WHEN OTHERS THEN 
				RESULTADOASIST := 'TRUE';
				MENSAJEASIST   := 'HA SIDO INGRESADO CON EXITO';
				DBMS_OUTPUT.PUT_LINE(''|| MENSAJEASIST)
				ROLLBACK; 
END; 
/



----------------------------------------------------
-- BORRAR ASISTENTE
----------------------------------------------------
CREATE OR REPLACE PROCEDURE QUITAR_CLIENTE(
	IDENTIFICADOR_ASIST IN ASISTENTE.ID_ASISTENTE%TYPE,

	---RETURN
	RESULTADOASISTENTE  OUT VARCHAR2,
	MENSAJEASISTENTE    OUT VARCHAR2
)

	IS DECLARE RUTASISTE     NUMBER, 
			   ID_USERASIST  NUMBER,
			   ID_ASISTE     NUMBER,
			   NOMBREASIST   VARCHAR2,
			   DIRECASIST    VARCHAR2,
			   TELASIST      NUMBER

	--CURSOR PARA ENCONTRAR LOS VALORES APARTIR DEL RUT DEL CLIENTE
	DECLARE CURSOR CURSOR_BORROASIST
	IS 
		SELECT *
		FROM ASISTENTE 
		WHEN ID_ASISTENTE = IDENTIFICADOR_ASIST
		BEGIN 
			OPEN CURSOR_BORROASIST;
			FETCH CURSOR_BORROASIST INTO ID_ASISTE, ID_USERASIST, RUTASISTE, NOMBREASIST, DIRECASIST, TELASIST 

-----------------BORRADO DE RUT ASISTENTE
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
			DELETE FROM ASISTENTE WHERE RUT = RUTASISTE;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJEASIST = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJEASIST = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO DE NOMBRE ASISTENTE
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
			DELETE FROM ASISTENTE WHERE NOMBREASISTENTE = NOMBREASIST;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJEASIST = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJEASIST = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO DE DIRECCION ASISTENTE
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
			DELETE FROM ASISTENTE WHERE DIRECCION = DIRECASIST;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJEASIST = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJEASIST = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO DE TELEFONO ASISTENTE
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
			DELETE FROM ASISTENTE WHERE TELEFONO = TELASIST;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJEASIST = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJEASIST = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO
/
----------------------------------------------------
-- EDITAR ASISTENTE
----------------------------------------------------
--
--
--
---
--
--
--
--
--
--
--
--

----------------------------------------------------
-- INSERTAR SEGURO
----------------------------------------------------
CREATE OR REPLACE PROCEDURE CREAR_CLIENTE(
	IDSEGURO  IN SEGURO.ID_SEGURO%TYPE,
	VALORSEGU IN SEGURO.VALORSEGURO%TYPE,

	--RETURN RESULTADO 
	MENSAJESEGURO     OUT     VARCHAR2,
    RESULTASEGURO     OUT     VARCHAR2
)

	IS 	
	DECLARE SEGUROID NUMBER;

	AUX_IDSEG NUMBER NOT NULL; 
		CONSTRAINT AUX_IDSEG := IDSEGURO
			CHECK (AUX_IDSEG > 0):

	--CURSOR PARA CORROBORAR EL RUT DE LOS CLIENTES EXISTENTES 
	DECLARE CURSOR CURSOR_SEGURO
	IS 
		SELECT ID_SEGURO 
		FROM SEGURO
		BEGIN 
			OPEN CURSOR_SEGURO;
			FETCH CURSOR_SEGURO INTO SEGUROID
			IF AUX_IDSEG != SEGUROID
				THEN INSERT INTO SEGURO VALUES(AUX_IDSEG, VALORSEGU);
				COMMIT; 
				RESULTADOSEGURO := 'TRUE';
				MENSAJESEGURO   := 'SE HA INGRESADO EXITOSAMENTE';
			ELSE
				MENSAJESEGURO      := 'ESTE CLIENTE YA EXISTE'
				RESULTADOSEGURO    :=  'FALSE';
				DBMS_OUTPUT.PUT_LINE(''|| MENSAJESEGURO)
		EXCEPTION
			WHEN OTHERS THEN 
				RESULTADOSEGURO := 'TRUE';
				MENSAJESEGURO   := 'HA SIDO INGRESADO CON EXITO';
				DBMS_OUTPUT.PUT_LINE(''|| MENSAJESEGURO)
				ROLLBACK; 
END; 
/

----------------------------------------------------
-- BORRAR SEGURO
----------------------------------------------------
CREATE OR REPLACE PROCEDURE QUITAR_SEGURO(
	IDSEGURO IN SEGURO.ID_SEGURO%TYPE,

	---RETURN
	RESULTADOSEG  OUT VARCHAR2,
	MENSAJESEG    OUT VARCHAR2
)

	IS DECLARE VALORSEG     NUMBER,

	--CURSOR PARA ENCONTRAR ALOJAR LOS VALORES DE LA TABLA
	DECLARE CURSOR CURSOR_BORROSEGURO
	IS 
		SELECT VALORSEGURO
		FROM SEGURO 
		WHEN ID_SEGURO = IDSEGURO
		BEGIN 
			OPEN CURSOR_BORROSEGURO;
			FETCH CURSOR_BORROSEGURO INTO VALORSEG 

-----------------BORRADO DE VALOR SEG
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
			DELETE FROM SEGURO WHERE VALORSEGURO = VALORSEG;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJESEG = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJESEG = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO
/

----------------------------------------------------
-- EDITAR SEGURO
----------------------------------------------------
--
--
--
---
--
--
--
--
--
--
--


----------------------------------------------------
-- INSERTAR PAGO
----------------------------------------------------
CREATE OR REPLACE PROCEDURE CREAR_PAGO(
	IDPAGO    IN PAGO.ID_PAGO%TYPE,
	VALORPAGO IN PAGO.VALOR%TYPE,
	TIPOBARCO IN PAGO.TIPO%TYPE,
	
	--RETURN RESULTADO 
	MENSAJEPAGO     OUT     VARCHAR2,
    RESULTADOPAGO   OUT     VARCHAR2
)

	IS 	
	DECLARE RUTCLIENT NUMBER;

	AUX_RUTCLIENTE NUMBER NOT NULL; 
		CONSTRAINT AUX_RUTCLIENTE := RUTCLIENTE
			CHECK (AUX_RUTCLIENTE > 0):

	--CURSOR PARA CORROBORAR EL RUT DE LOS CLIENTES EXISTENTES 
	DECLARE CURSOR CURSOR_CLIENTE
	IS 
		SELECT RUT 
		FROM CLIENTE
		BEGIN 
			OPEN CURSOR_CLIENTE;
			FETCH CURSOR_CLIENTE INTO RUTCLIENT
			IF AUX_RUTCLIENTE != RUTCLIENT
				THEN INSERT INTO CLIENTE VALUES(AUX_RUTCLIENTE, IDUSERCLIENTE, NOMBRECLIENT, DIRECCIONCLIENTE, TELEFONOCLIENTE);
				COMMIT; 
				RESULTADOCLI := 'TRUE';
				MENSAJECLI   := 'SE HA INGRESADO EXITOSAMENTE';
			ELSE
				MENSAJECLI   := 'ESTE CLIENTE YA EXISTE'
				RESULTADOCLI    :=  'FALSE';
				DBMS_OUTPUT.PUT_LINE(''|| MENSAJECLI)
		EXCEPTION
			WHEN OTHERS THEN 
				RESULTADOCLI := 'TRUE';
				MENSAJECLI   := 'EL BARCO HA SIDO INGRESADO CON EXITO';
				DBMS_OUTPUT.PUT_LINE(''|| MENSAJECLI)
				ROLLBACK; 
END; 
/

----------------------------------------------------
-- BORRAR PAGO
----------------------------------------------------
CREATE OR REPLACE PROCEDURE QUITAR_PAGO(
	IDPAGO_ IN PAGO.ID_PAGO%TYPE,

	---RETURN
	RESULTADOPAGO_  OUT VARCHAR2,
)

	IS DECLARE VALORPAGO_     NUMBER, 
			   TIPOBARCOPAGO_ VARCHAR2,
			   

	--CURSOR PARA ENCONTRAR LOS VALORES APARTIR DEL RUT DEL CLIENTE
	DECLARE CURSOR CURSOR_BORROCLIENTE
	IS 
		SELECT VALOR, TIPO
		FROM PAGO 
		WHEN ID_PAGO = IDPAGO_
		BEGIN 
			OPEN CURSOR_BORROCLIENTE;
			FETCH CURSOR_BORROCLIENTE INTO VALORPAGO_, TIPOBARCOPAGO_ 

-----------------BORRADO DE VALOR
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
			DELETE FROM PAGO WHERE VALOR = VALORPAGO_;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJEPAGO_ = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJEPAGO_ = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO DE TIPOBARCO
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
			DELETE FROM PAGO WHERE TIPO = TIPOPAGO_;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJEPAGO_ = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJEPAGO_ = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO
/
----------------------------------------------------
-- EDITAR PAGO
----------------------------------------------------
--
--
--
---
--
--
--
--
--
--
--
--

----------------------------------------------------
-- INSERTAR ENCUESTA
----------------------------------------------------
CREATE OR REPLACE PROCEDURE CREAR_ENCUESTA(
	IDENCUESTA IN ENCUESTA.ID_ENCUESTA%TYPE,
	VALORAR    IN ENCUESTA.VALORACION%TYPE,
	COMENT     IN ENCUESTA.COMENTARIO%TYPE,

	--RETURN RESULTADO 
	MENSAJEENCUESTA     OUT     VARCHAR2,
    RESULTAENCUESTA   OUT     VARCHAR2
)

	IS 	
	DECLARE IDENCUE NUMBER;

	AUX_ENCUESTA NUMBER NOT NULL; 
		CONSTRAINT AUX_ENCUESTA := IDENCUESTA
			CHECK (AUX_ENCUESTA > 0):

	--CURSOR PARA CORROBORAR EL RUT DE LOS CLIENTES EXISTENTES 
	DECLARE CURSOR CURSOR_ENCUESTA
	IS 
		SELECT ID_ENCUESTA 
		FROM ENCUESTA
		BEGIN 
			OPEN CURSOR_ENCUESTA;
			FETCH CURSOR_ENCUESTA INTO IDENCUE
			IF AUX_ENCUESTA != IDENCUE
				THEN INSERT INTO ENCUESTA(IN_ENCUESTA,VALORACION,COMENTARIO) VALUES(IDENCUESTA, VALORAR, COMENT);
				COMMIT; 
				RESULTADOENCUESTA := 'TRUE';
				MENSAJEENCUESTA   := 'SE HA INGRESADO EXITOSAMENTE';
			ELSE
				MENSAJEENCUESTA      := 'ESTE CLIENTE YA EXISTE'
				RESULTADOENCUESTA    :=  'FALSE';
				DBMS_OUTPUT.PUT_LINE(''|| MENSAJEENCUESTA)
		EXCEPTION
			WHEN OTHERS THEN 
				RESULTADOENCUESTA := 'TRUE';
				MENSAJEENCUESTA   := 'HA SIDO INGRESADO CON EXITO';
				DBMS_OUTPUT.PUT_LINE(''|| MENSAJEENCUESTA)
				ROLLBACK; 
END; 
/

----------------------------------------------------
-- BORRAR ENCUESTA
----------------------------------------------------
CREATE OR REPLACE PROCEDURE QUITAR_ENCUESTA(
	IDENCUESTA_ IN ENCUESTA.ID_ENCUESTA%TYPE,

	---RETURN
	RESULTADOENCUESTA_  OUT VARCHAR2,
	MENSAJEENCUESTA_    OUT VARCHAR2
)

	IS DECLARE IDENC      NUMBER, 
			   VALORARENC NUMBER,
			   COMENTENC  VARCHAR2,


	--CURSOR PARA ENCONTRAR LOS VALORES APARTIR DEL RUT DEL CLIENTE
	DECLARE CURSOR CURSOR_BORROENCUESTA
	IS 
		SELECT ID_ENCUESTA, VALORACION, COMENTARIO
		FROM ENCUESTA 
		WHEN ID_ENCUESTA = IDENCUESTA_
		BEGIN 
			OPEN CURSOR_BORROENCUESTA;
			FETCH CURSOR_BORROENCUESTA INTO IDENC, VALORARENC, COMENTENC 

-----------------BORRADO DE VALORACION
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
			DELETE FROM ENCUESTA WHERE VALORACION = VALORARENC;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJEENCUESTA_ = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJEENCUESTA_ = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO DE COMENTARIO
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
			DELETE FROM ENCUESTA WHERE COMENTARIO = COMENTENC;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJEENCUESTA_ = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJEENCUESTA_ = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO


---------------------------------------------------
-- EDITAR ENCUESTA
----------------------------------------------------
--
--
--
---
--
--
--
--
--
--
--
--

----------------------------------------------------
-- BORRAR ARRIENDO 
----------------------------------------------------
CREATE OR REPLACE PROCEDURE BORRAR_ARRIENDO(
	IDARRIENDO IN ARRIENDO.ID_ARRIENDO%TYPE,

	---RETURN
	MENSAJEARRIENDO   OUT       VARCHAR2,
    RESULTADO         OUT       VARCHAR2,
)

	IS DECLARE LUGARRETIRO     VARCHAR2, 
			   LUGARENTREGA    VARCHAR2,
			   FECHARETIRO     DATE,
			   FECHAENTREGA    DATE,
			   HORARETIRO      DATE,
			   HORAENTREGA     DATE,
			   VALORARRIENDO   NUMBER, 
			   ESTADOARRIENDO  NUMBER

	--CURSOR PARA ENCONTRAR LOS VALORES APARTIR DEL RUT DEL CLIENTE
	DECLARE CURSOR CURSOR_BORROARRIENDO
		SELECT LUGAR_RETIRO,LUGAR_ENTREGA,FECHA_RETIRO, FECHA_ENTREGA, HORA_RETIRO,HORA_ENTREGA,VALOR,ESTADO
		FROM ARRIENDO 
		WHERE ID_ARRIENDO = IDARRIENDO
		BEGIN 
            RESULTADO := 'TRUE';
			OPEN CURSOR_BORROARRIENDO;
			FETCH CURSOR_BORROARRIENDO INTO LUGARRETIRO, LUGARENTREGA, FECHARETIRO ,FECHAENTREGA, HORARETIRO, HORAENTREGA, VALORARRIENDO, ESTADOARRIENDO 
            
-----------------BORRADO LUGAR RETIRO
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
            RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO WHERE LUGAR_RETIRO = LUGARRETIRO;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                RESULTADO := 'FALSE';
				SET MENSAJEARRIENDO = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                RESULTADO := 'TRUE';
				SET MENSAJEARRIENDO = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO LUGAR ENTREGA
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
             RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO WHERE LUGAR_ENTREGA = LUGARENTREGA;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                 RESULTADO := 'FALSE';
				SET MENSAJEARRIENDO = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                RESULTADO := 'TRUE';
				SET MENSAJEARRIENDO = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO FECHA RETIRO
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
            RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO WHERE FECHA_RETIRO = FECHARETIRO;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                RESULTADO := 'FALSE';
				SET MENSAJEARRIENDO = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                RESULTADO := 'TRUE';
				SET MENSAJEARRIENDO = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO


-----------------BORRADO FECHA ENTREGA
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
            RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO WHERE FECHA_ENTREGA = FECHAENTREGA;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                RESULTADO := 'FALSE';
				SET MENSAJEARRIENDO = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                RESULTADO := 'TRUE';
				SET MENSAJEARRIENDO = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO


-----------------BORRADO HORA RETIRO
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
            RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO WHERE HORA_RETIRO = HORARETIRO;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                 RESULTADO := 'FALSE';
				SET MENSAJEARRIENDO = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                RESULTADO := 'TRUE';
				SET MENSAJEARRIENDO = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO HORA RETIRO
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
            RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO WHERE HORA_ENTREGA = HORAENTREGA;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                 RESULTADO := 'FALSE';
				SET MENSAJEARRIENDO = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                 RESULTADO := 'TRUE';
				SET MENSAJEARRIENDO = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO VALOR
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
             RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO WHERE VALOR = VALORARRIENDO;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                 RESULTADO := 'FALSE';
				SET MENSAJEARRIENDO = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                RESULTADO := 'TRUE';
				SET MENSAJEARRIENDO = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO ESTADO
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
            RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO WHERE ESTADO = ESTADOARRIENDO;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                RESULTADO := 'FALSE';
				SET MENSAJEARRIENDO = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                RESULTADO := 'TRUE';
				SET MENSAJEARRIENDO = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

----------------------------------------------------
-- EDITAR PAGO
----------------------------------------------------
CREATE OR REPLACE PROCEDURE UPDATE_PAGO(
	ID_PAGO    IN PAGO.ID_PAGO%TYPE,
	VALOR_PAGO IN PAGO.VALOR%TYPE,
	TIPO_BARCO IN PAGO.TIPO%TYPE,
	MENSAJEASIST   		  OUT       VARCHAR2,
    RESULTADO         OUT       VARCHAR2,
	

)


BEGIN
	BEGIN TRY
		QUITAR_PAGO(ID_PAGO);
		RESULTADO:'TRUE';
		MENSAJEASIST: 'EDITADO CORRECTAMENTE'
		THEN CREAR_PAGO(ID_PAGO, VALOR_PAGO, TIPO_BARCO);
	END TRY
	BEGIN CATCH 
		
			EXEC usp_report_error;
			IF (XACT_STATE()) = -1
			BEGIN
				SET MENSAJEASIST = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJEASIST = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH
END;
/

---------------------------------------------------
-- EDITAR ARRIENDO
----------------------------------------------------
--
--
--
---
--
--
--
--
--
--
--
--

----------------------------------------------------
-- BORRAR ARRIENDO DISPONIBLE
----------------------------------------------------
CREATE OR REPLACE PROCEDURE QUITAR_ARRIENDODISPONIBLES(
	ID_ARRIENDODISP IN ARRIENDO_DISPONIBLES.ID_ARRIENDO%TYPE,

	---RETURN
	MENSAJEARRIENDODISP    OUT VARCHAR2;
    RESULTADO              OUT VARCHAR2
)

	IS DECLARE LUGARRETIRO_AD     VARCHAR2, 
			   LUGARENTREGA_AD    VARCHAR2,
			   FECHARETIRO_AD     DATE,
			   FECHAENTREGA_AD    DATE,
			   HORARETIRO_AD      DATE,
			   HORAENTREGA_AD     DATE,
			  

	--CURSOR PARA ENCONTRAR LOS VALORES 
	DECLARE CURSOR CURSOR_BORROARRIENDODIPS
	IS 
		SELECT LUGAR_RETIRO,LUGAR_ENTREGA,FECHA_RETIRO, FECHA_ENTREGA, HORA_RETIRO,HORA_ENTREGA
		FROM ARRIENDO_DISPONIBLES
		WHEN ID_ARRIENDO_DISPONIBLE = ID_ARRIENDODISP
		BEGIN 
			OPEN CURSOR_BORROARRIENDODIPS;
			FETCH CURSOR_BORROARRIENDODIPS INTO LUGARRETIRO_AD, LUGARENTREGA_AD, FECHARETIRO_AD, FECHAENTREGA_AD, HORARETIRO_AD, HORAENTREGA_AD

-----------------BORRADO DE LUGAR RETIRO
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
            RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO_DISPONIBLES WHERE LUGAR_RETIRO = LUGARRETIRO_AD;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                RESULTADO := 'FALSE';
				SET MENSAJEARRIENDODISP = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
				SET MENSAJEARRIENDODISP = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO DE LUGAR ENTREGA
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
            RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO_DISPONIBLES WHERE LUGAR_ENTREGA = LUGARENTREGA_AD;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                RESULTADO := 'FALSE';
				SET MENSAJEARRIENDODISP = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                RESULTADO := 'TRUE';
				SET MENSAJEARRIENDODISP = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO DE FECHA RETIRO
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
            RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO_DISPONIBLES WHERE FECHA_RETIRO = FECHARETIRO_AD;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                RESULTADO := 'FALSE';
				SET MENSAJEARRIENDODISP = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                RESULTADO := 'TRUE';
				SET MENSAJEARRIENDODISP = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO DE FECHA ENTREGA
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
            RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO_DISPONIBLES WHERE FECHA_ENTREGA = FECHAENTREGA_AD;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                RESULTADO := 'FALSE';
				SET MENSAJEARRIENDODISP = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                RESULTADO := 'TRUE';
				SET MENSAJEARRIENDODISP = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO DE HORA RETIRO
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
            RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO_DISPONIBLES WHERE HORA_RETIRO = HORARETIRO_AD;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                RESULTADO := 'FALSE';
				SET MENSAJEARRIENDODISP = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                RESULTADO := 'TRUE';
				SET MENSAJEARRIENDODISP = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

-----------------BORRADO DE HORA ENTREGA
	AS 
	BEGIN 
		BEGIN TRY 
			BEGIN TRANSACCTION;
            RESULTADO := 'TRUE';
			DELETE FROM ARRIENDO_DISPONIBLES WHERE HORA_ENTREGA = HORAENTREGA_AD;
			COMMIT TRANSACCTION;
		END TRY

		BEGIN CATCH 
			EXEC usp_report_error;:
			IF (XACT_STATE()) = -1
			BEGIN
                RESULTADO := 'FALSE';
				SET MENSAJEARRIENDODISP = 'ESTA ACCION SE ENCUENTRA EN UN ESTADO COMPREMETIDO'
				ROLLBACK TRANSACCTION;
			END; 

			IF (XACT_STATE()) = 1
			BEGIN 
                RESULTADO := 'TRUE';
				SET MENSAJEARRIENDODISP = 'ESTA ACCION NO SE ENCUENTRA COMPREMETIDA'
				COMMIT TRANSACCTION; 
			END; 
		END CATCH 
	END;
	GO

